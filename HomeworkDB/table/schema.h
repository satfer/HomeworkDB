...
class Schema {
	// 属性->类型和空间大小
public:
	Schema() = default; //虽然想把它delete掉, 但是Table::Table(const std::string &s)没法一开始就初始化schema_导致要调用Schema::Schema()
...
public:
	...
	~TypeHandler() = default;
	virtual void read() = 0;
	virtual void toWrite() = 0; // 即使是虚函数, 重写也要求返回类型相同或协同(为子类), 只能把读写的东西放在参数里了
};
